...
def jpeg_gris(imagen_gray):
    ...
    for i in range(numBloc):
        for w in range(8):
            for z in range(8):
                BlockImage[i,w,z] /= Q_Luminance[w,z]
                BlockImage[i,w,z] = round(BlockImage[i,w,z],0)
                
    #Compresion ratio:
    ImagenCuantizada = reshape2(m,n,BlockImage)[:OrigM,:OrigN]
    coef = m*n
    ...
    imagenInv = reshape2(m,n,BlockImage)
